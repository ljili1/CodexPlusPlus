...
/// 当开启「统一模型目录」时，若 `model` 以某个已配置供应商的显示名前缀开头，
/// 返回对应的供应商 id 与去掉前缀后的模型名。大小写不敏感时再次尝试匹配。
pub fn resolve_prefixed_model(settings: &BackendSettings, model: &str) -> Option<(String, String)> {
    ...
    for profile in &settings.relay_profiles {
        ...
        if !prefix.is_empty() && model.starts_with(&prefix) {
            return Some((profile.id.clone(), model[prefix.len()..].to_string()));
        }
    }
    let lower = model.to_lowercase();
    for profile in &settings.relay_profiles {
        let prefix = member_model_prefix(&profile.name, &profile.id).to_lowercase();
        if !prefix.is_empty() && lower.starts_with(&prefix) {
            let skip = prefix.chars().count();
            let rest: String = model.chars().skip(skip).collect();
            return Some((profile.id.clone(), rest));
        }
    }
    None
}
