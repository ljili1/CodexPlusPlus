...
export const EN_PLAIN: Record<string, string> = {
  ...
  "切换到跟随系统": "Switch to system theme",
  ...
  "config.toml / auth.json / .sandbox-secrets": "config.toml / auth.json / .sandbox-secrets",
  "对话历史": "Conversation history",
  "sessions / archived_sessions / skills": "sessions / archived_sessions / skills",
  "记忆目标状态库": "Memory / goals / state store",
  "memories / goals / state 等 .sqlite": "memories / goals / state .sqlite files",
  "界面设置": "UI settings",
  "settings.json / dream-skin 壁纸": "settings.json / dream-skin wallpapers",
  "日志": "Logs",
  "logs_2.sqlite，可能较大": "logs_2.sqlite, may be large",
  "默认不选": "Off by default",
  "导出配置": "Export config",
  "导入配置": "Import config",
  "导入配置会覆盖当前设置、密钥与对话历史。备份包以明文存储密钥，请确认文件来源可信后再继续。是否继续？": "Importing overwrites current settings, credentials and conversation history. The backup stores secrets in plain text; only continue if you trust the file source.",
  "全局记忆": "Global memory",
  "开启后，这里的内容会写入 Codex 的 AGENTS.md，作为所有会话的全局指令；关闭并清空时由 Codex++ 自动移除（不影响你手动维护的 AGENTS.md）。": "When enabled, this content is written to Codex's AGENTS.md as a global instruction for all sessions; disabling or emptying it lets Codex++ remove it automatically (your manually maintained AGENTS.md is untouched).",
  "启用全局记忆": "Enable global memory",
  "支持多行；留空等同于关闭。": "Multi-line supported; empty means disabled.",
  "记忆内容": "Memory content",
  "在此输入需要 Codex 长期记住的内容，例如偏好、约定、项目背景……": "Enter what you want Codex to remember long-term, e.g. preferences, conventions, project background...",
};
